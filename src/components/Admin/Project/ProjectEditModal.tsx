...
import { IUpdateProjectDto, IProjectContactDto, IProjectRoleDto } from "@/models/ProjectModels";
...
const ProjectEditModal = (modalInfo: IProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (modalInfo.showModal && modalInfo.projectId) {
        try {
          const response = await ProjectDataService.getProjectById(modalInfo.projectId);
    ...
    fetchData();
  }, [modalInfo.showModal, modalInfo.projectId]);
  ...
  const onUpdateProject = () => {
    ...
    const postAct = () => {
      modalInfo.onClose();
      modalInfo.onRefresh();
      setAlertMessage(SystemAlertConstants.UpdateProjectSuccessConstant);
    };

    ProjectDataService.updateProject(modalInfo.projectId, project)
      .then(postAct)
      .catch(() => setAlertMessage(SystemAlertConstants.UpdateProjectErrorConstant));
  };
            ...
            <MuiMaterial.Stack spacing={2}>
              ...
              <MuiMaterial.Box role="tabpanel">
                <MuiMaterial.Collapse in={activeTab === "contacts"}>
                  {/* Реализация контактов здесь */}
                </MuiMaterial.Collapse>
                <MuiMaterial.Collapse in={activeTab === "roles"}>
                  {/* Реализация ролей здесь */}
                </MuiMaterial.Collapse>
              </MuiMaterial.Box>

              <MuiMaterial.Button variant="outlined" onClick={onUpdateProject}>
                Сохранить
              </MuiMaterial.Button>
            </MuiMaterial.Stack>
          </MuiMaterial.DialogContent>
